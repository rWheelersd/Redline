...
(function () {
  ...
  const COPY = "Place holder until i meet until we meet on Sunday.";
  ...
  function demoEvents(today) {
    ...
    return [
      { id: "demo-1", date: dateKey(addDays(today, 1)), time: "09:00", duration: 120, tone: "purple" },
      { id: "demo-2", date: dateKey(addDays(today, 3)), time: "14:00", duration: 60, tone: "blue" },
      { id: "demo-3", date: dateKey(nextSunday), time: "10:00", duration: 90, tone: "green" }
    ];
  }
  ...
  function loadEvents() {
    ...
    return demoEvents(localDay(new Date().getFullYear(), new Date().getMonth(), new Date().getDate()));
  }
  ...
  deleteButton.addEventListener("click", () => {
    if (!state.editingId || !window.confirm(COPY)) return;
